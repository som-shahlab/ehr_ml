...
class DoctorAITask(nn.Module):
    ...
    def __init__(self, config, data_config, info):
        ...
        if config['simple_constant_baseline']:
            self.output_code_embedding = nn.Embedding(len(data_config['leaf_code_map']), 1)
        else:
            self.output_code_embedding = nn.Embedding(len(data_config['leaf_code_map']), self.config['size'] + 1)
    ...
    def forward(self, rnn_output, data):
        ...
        loss = nn.functional.binary_cross_entropy(
            probabilities, non_text_expected_output, 
            weight=non_text_output_weights, reduction='sum')

        return final, loss

    @classmethod
    def positive_codes_iterator(cls, data_config, info, ontologies, patient, mask_before=None):
        ...
        for day in patient.days:
            ...
            if mask_before is not None: 
                this_date = datetime.date(year=day.date.year, month=day.date.month, day=day.date.day)
    ...
    def compute_leaf_code_map(cls, data_config, info, ontologies):
        ...
        for code in info['valid_code_map'].keys():
            ...
            if code_type in {'ATC'}:
                ...
                valid_children = set(children) & set(info['valid_code_map'].keys())
    ...
    @classmethod
    def convert_samples_to_variables(cls, data_config, info, ontologies, patients, mask_before=None):
        """
        Name is confusing but this constructs _targets_ from next day codes and terms. 
        """

        maximum_length = max(sum(info.get('use_terms') or len(day.observations) > 0 for day in x.days) for x in patients)

        non_text_expected_output = torch.ByteTensor(
            len(patients), maximum_length, len(data_config['leaf_code_map'])).fill_(0)
        non_text_output_weights = torch.ByteTensor(
            len(patients), maximum_length, len(data_config['leaf_code_map'])).fill_(0)

        for i, patient in enumerate(patients):
            for j, positive_codes in cls.positive_codes_iterator(data_config, info, ontologies, patient, mask_before=mask_before):
                any_added = False
