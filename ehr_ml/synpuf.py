...
def clean_synpuf() -> None:
    ...
    parser.add_argument(
        "synpuf_source_dir",
        type=str,
        help="Path of the source synpuf folder",
    )
    ...
    with urllib.request.urlopen(r"https://raw.githubusercontent.com/OHDSI/CommonDataModel/v5.2.2/PostgreSQL/OMOP%20CDM%20ddl%20-%20PostgreSQL.sql") as f:
    ...
    for name in os.listdir(args.synpuf_source_dir):
        ...
        with open(os.path.join(args.synpuf_source_dir, name)) as i:
            with gzip.open(os.path.join(args.synpuf_target_dir, name.lower() + '.gz'), 'wt') as o:
