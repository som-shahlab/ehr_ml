...
def create_dummy_patient(patient_id, days_info):
    ...
    for date, observations, observations_with_values in days_info:
        days.append(DummyDay(
            age = (
                datetime.date(*date) - 
                datetime.date(*days_info[0][0])
            ).days,
            date = datetime.date(*date),
            observations = observations,
            observations_with_values = [DummyObservationWithValue(code=c, numeric_value=v, is_text=False) for c,v in observations_with_values.items()]
        ))
    return DummyPatient(patient_id=patient_id, days=days)

@pytest.fixture(scope='session')
def dummy_patient():
    return create_dummy_patient(234123, [
        ((1994, 2, 9), [0, 4, 12], {}),
...
def dummy_timeline():
    ...
    class DummyTimeline:
        ...
        def get_patient(self, patient_id, end_date=None):
            return create_dummy_patient(patient_id, [
                ((1994, 2, 9), [0, 4, 12], {}),
                ((1997, 2, 9), [0, 34, 129], {909: 0}),
                ((2000, 2, 9), [0, 34, 129], {909: 2}),
            ])
