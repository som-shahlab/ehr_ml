...
from ehr_ml.featurizer import AgeFeaturizer, ColumnValue, \
    CountFeaturizer, FeaturizerList
...
def test_count_bins_featurizer(dummy_patient, dummy_timeline, dummy_ontologies):
    featurizer = CountFeaturizer(dummy_timeline, dummy_ontologies, time_bins=[365 * 2, None])
...
def test_complete_featurization(dummy_patient, dummy_timeline, dummy_ontologies):
    age = AgeFeaturizer()
    ...
    featurizersAge.train_featurizers(
        dummy_timeline, labeler)
    featurizersCount.train_featurizers(
        dummy_timeline, labeler)
    featurizersBoth.train_featurizers(
        dummy_timeline, labeler)
    ...
    the_same = full_matrix.toarray() == scipy.sparse.hstack(
        (age_matrix, count_matrix)).toarray()
