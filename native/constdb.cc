...
ConstdbReader::ConstdbReader(const char* filename, bool read_all, bool old_format) {
    ...
    if (read_all) {
        int error = posix_fadvise(fd, 0, 0, POSIX_FADV_SEQUENTIAL | POSIX_FADV_WILLNEED);
        ...
        mmap_data = (const char*) mmap(nullptr, length, PROT_READ, MAP_SHARED | MAP_POPULATE, fd, 0);
    } else {
        mmap_data = (const char*) mmap(nullptr, length, PROT_READ, MAP_SHARED, fd, 0);
    }
    ...
    while (current_location < length) {
        if (!old_format) {
            ...
            auto offset = std::make_pair((const char*)(mmap_data + current_offset), size);
            current_offset += size;
            ...
            if (is_int) {
                ...
                std::string key_str = std::string((char*) mmap_data + current_location, key_val);
            ...
            auto offset = std::make_pair((const char*)(mmap_data + begin), size);
            current_offset += size;
...
ConstdbReader::ConstdbReader(ConstdbReader&& other): fd(other.fd), mmap_data(other.mmap_data), int_offsets(other.int_offsets), str_offsets(other.str_offsets), int_keys(other.int_keys)  {
...
std::pair<const char*, size_t> ConstdbReader::get_str(const std::string& key) const {
    if (mmap_data == 0) {
        printf("Invalid reader?\n");
        exit(-1);
    }
...
ConstdbWriter::ConstdbWriter(const char* filename): writer(filename, std::ios_base::out|std::ios_base::binary) {}

void ConstdbWriter::add_int(int32_t key, const char* bytes, int32_t length) {
    writer.write(bytes, length);
    entries.push_back(std::make_pair(std::variant<int32_t, std::string>(key), length));
}

void ConstdbWriter::add_str(std::string key, const char* bytes, int32_t length) {
    writer.write(bytes, length);
    entries.push_back(std::make_pair(std::variant<int32_t, std::string>(key), length));
}
...
ConstdbWriter::~ConstdbWriter() {
    ...
    for (auto& entry : entries) {
        ...
        std::visit([this, length](auto&& arg) {
