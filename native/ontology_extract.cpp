...
const std::string* try_to_recover(
  ...
  if (new_id_candidates.size() == 0) {
    ...
    for (std::string terminology : map_terminology_type(info.vocabulary_id)) {
      auto aui_iter =
          code_to_aui_map.find(std::make_pair(terminology, info.concept_code));
  ...
  if (aui_to_subwords_map.find(aui) == std::end(aui_to_subwords_map)) {
    ...
    if (parents_iter != std::end(aui_to_parents_map)) {
      for (const auto& parent_aui : parents_iter->second) {
        ...
        for (uint32_t subword : compute_subwords(
                 parent_aui, aui_to_code_map, aui_to_parents_map,
                 aui_to_subwords_map, code_to_parents_map, dictionary)) {
...
int main() {
  ...
  for (uint32_t i = 0; i < words.size(); i++) {
    ...
    if (result == nullptr && (parts[0] == "CPT4" || parts[0] == "ICD10CM")) {
    ...
    if (result == nullptr) {
      ...
      subwords = compute_subwords(*result, aui_to_code_map, aui_to_parents_map,
