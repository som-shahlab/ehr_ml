...
class ClaimItemConverter : public Converter {
 public:
  ...
  void augment_day(Metadata& meta, RawPatientRecord& patient_record,
    ...
    patient_record.claim_observations.push_back(std::make_pair(claim_id, code));
  }
 ...
 public:
  ...
  void augment_day(Metadata& meta, RawPatientRecord& patient_record,
    ...
    if (row[2] != "") {
      ...
      std::cout << "Got a facility detail with no valid date? " << filename
                << " " << row[0] << std::endl;
      abort();
    }
    ...
    if (row[1] != "" && row[1] != "00000") {
      uint32_t code =
          meta.dictionary.map_or_add(absl::Substitute("MainProc/$0", row[1]));
 ...
 public:
  ...
  void augment_day(Metadata& meta, RawPatientRecord& patient_record,
    ...
    if (row[4] != "" && row[4] != "00000") {
      uint32_t proc =
          meta.dictionary.map_or_add(absl::Substitute("MainProc/$0", row[4]));
...
class DetailMemberConverter : public Converter {
 public:
  ...
  void augment_day(Metadata& meta, RawPatientRecord& patient_record,
    ...
    if (!patient_record.birth_date || *patient_record.birth_date > birth_date) {
...
class HeapItem {
 public:
  ...
  bool operator<(const HeapItem& second) const {
    ...
    return first_person_id.value_or(limit) > second_person_id.value_or(limit);
  }
...
class Merger {
 public:
  ...
  WriterItem operator()() {
    while (true) {
      std::optional<uint64_t> possible_person_id = heap.front().get_person_id();

      if (possible_person_id.has_value()) {
        RawPatientRecord total_record;
        total_record.person_id = possible_person_id.value();
        total_record.birth_date = absl::CivilDay(9999, 1, 1);  // Dummy value
        ...
        while (heap.front().get_person_id() == possible_person_id) {
          ...
          RawPatientRecord& record = std::get<RawPatientRecord>(queue_item);
          ...
          auto offset = [&](uint32_t val) { return (val << shift) + index; };
          ...
          for (const auto& obs : record.observations_with_values) {
            ...
            if (obs_with_value.is_text) {
              obs_with_value.text_value = offset(obs_with_value.text_value);
            }
        ...
        for (const auto& claim_obs : total_record.claim_observations) {
          ...
          claim_iter = std::find_if(claim_iter, std::end(total_record.claims),
                                    [&](const auto& claim) {
                                      return claim.first == claim_obs.first;
                                    });

          if (claim_iter == std::end(total_record.claims)) {
            std::cout << "Could not find claim??? " << total_record.person_id
                      << " " << claim_name << std::endl;
            for (const auto& c_index : contributing_indexes) {
              char thread_name[16];
              pthread_getname_np(
                  converter_threads[c_index].first.native_handle(), thread_name,
                  sizeof(thread_name));
        ...
        if (!total_record.birth_date) {
          ...
          if (rand() % lost_patients == 0) {
            std::cout << "You have a patient without a birth date?? "
                      << total_record.person_id << " so far " << lost_patients
        ...
        if (contributing_indexes.size() == 1) {
          ...
          if (rand() % ignored_patients == 0) {
            std::cout << "You are ignoring a patient " << total_record.person_id
                      << " so far " << ignored_patients << " out of "
        ...
        for (const auto& observ : total_record.birth_observations) {
          final_record.observations.push_back(std::make_pair(0, observ));
        }

        for (const auto& observ : total_record.observations_with_values) {
          final_record.observations_with_values.push_back(std::make_pair(
              observ.first - final_record.birth_date, observ.second));
        }
        ...
        for (auto& heap_item : heap) {
          ...
          auto process =
              [&](const TermDictionary& source,
                  std::vector<std::pair<std::string, uint32_t>>& target) {
        ...
        total_metadata.dictionary = TermDictionary(std::move(dictionary));
        total_metadata.value_dictionary =
            TermDictionary(std::move(value_dictionary));

        std::cout << "Done with " << lost_patients << " lost patients and "
                  << ignored_patients << " ignored patients out of "
                  << total_patients << std::endl;
 ...
 private:
  ...
  std::vector<std::pair<std::thread, std::shared_ptr<Queue>>> converter_threads;
};
