...
class ExtractReaderIterator {
 public:
  ...
  bool process_patient(uint32_t patient_id, F f) {
    ...
    const uint8_t* main_data = (const uint8_t*)patient_data + sizeof(uint32_t);
...
class ExtractReader {
 public:
  ...
  absl::Span<const uint64_t> get_original_patient_ids() const {
    ...
    return absl::Span<const uint64_t>(original_patient_ids_ptr, *num_patients);
  }

  ExtractReaderIterator iter() const { return ExtractReaderIterator(&reader); }
  ...
  const TermDictionary& get_value_dictionary() {
    if (!value_dictionary) {
      auto [val_dict_start, val_dict_size] = reader.get_str("value_dictionary");
...
class OntologyReader {
 public:
  OntologyReader(const char* path) : reader(path, false) {
    for (uint32_t subword = 0; subword < get_dictionary().size(); subword++) {
  ...
  absl::Span<const uint32_t> get_all_parents(uint32_t subword) {
    ...
    if (iter == std::end(all_parents_map)) {
      ...
      auto res =
          all_parents_map.insert(std::make_pair(subword, std::move(parents)));
      iter = res.first;
    }
 ...
 private:
  ...
  get_inverse_map() {
    if (!inverse_map) {
      ...
      absl::Span<const uint32_t> words_with_subwords((const uint32_t*)ptr,
                                                     size / sizeof(uint32_t));
...
class Index {
 public:
  ...
  std::vector<uint32_t> get_patient_ids(uint32_t term) {
    ...
    const uint8_t* main_data = (const uint8_t*)(iter_ptr + sizeof(uint32_t));

    std::vector<uint32_t> result(*size_ptr);

    size_t bytes_read = streamvbyte_decode(main_data, result.data(), *size_ptr);
