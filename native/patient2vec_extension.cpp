 ...
 public:
  ...
  void add(T value) {
    ...
    if (num_samples_seen <= max_samples) {
      ...
      std::uniform_int_distribution<> index_distribution(0, max_samples - 1);
...
std::string create_info(const char* timeline_path, const char* ontology_path,
            ...
            for (const auto& obs_with_value : observations_with_values) {
              ...
              if (obs_with_value.is_text) {
                auto sampler = text_lab_samples.find_or_insert(
                    obs_with_value.code, ReservoirSampler<uint32_t>(10000));
                sampler->add(obs_with_value.text_value);
              } else {
                auto sampler = numeric_lab_samples.find_or_insert(
                    obs_with_value.code, ReservoirSampler<float>(10000));
  ...
  for (uint32_t code = 0; code < code_counts.size(); code++) {
    ...
    bool has_root = std::find(std::begin(all_parents), std::end(all_parents),
  ...
  for (auto& item : numeric_map) {
    ...
    for (int i = 0; i <= num_splits; i++) {
      ...
      if (i == 0) {
        ...
        current_value = item.second[(item.second.size() * i) / num_splits];
      }
    ...
    if (numeric_ranges.size() > 2) {
      ...
      serialized[std::to_string(item.first)]["numeric_ranges"] = numeric_ranges;
  ...
  for (auto& item : text_map) {
    ...
    for (uint32_t text_value : item.second) {
      std::string normalized = normalize_lab_value(
          std::string(*reader.get_value_dictionary().get_word(text_value)));
      if (normalized == "") {
        continue;
      }
...
class StrideDataset {
 public:
  ...
  StrideDataset(const char* ontology_path, const char* compressed_extract_path,
                const char* info_path, py::tuple train_data, py::tuple val_data)
      : StrideDataset(ontology_path, compressed_extract_path, info_path) {
...
struct StrideDatasetBatch {
  ...
  // std::vector<uint8_t> censor_masks;
  // std::vector<uint8_t> event_masks;
...
class StrideDatasetIterator {
 public:
  ...
  void special_targets(StrideDatasetBatch& batch, uint32_t patient_id,
    ...
    if (target_index != nullptr && term == "ICD10CM/E00-E89") {
      for (size_t bin_index = 0; bin_index < parent.ends.size(); bin_index++) {
        ...
        uint64_t index =
            *target_index * 2 * 20 + (is_start ? 0 : 1) * 20 + bin_index;

        if (is_final) {
          if (is_censor) {
            float factor = (target_age - start) / ((float)(end - start));
            batch.target_indices.push_back({patient_index, index});
            batch.target_labels.push_back(0);
            batch.target_factors.push_back(factor);
          } else {
            batch.target_nonfactor_indices.push_back({patient_index, index});
  ...
  void fill_in_batch(std::mt19937_64& batch_rng,
    ...
    // parent.num_valid_targets; batch.target_times.resize(num_target_entries,
    // 1); batch.censor_masks.resize(num_target_entries, 0);
    ...
    for (int i = 0; i < (end_index - start_index); i++) {
      ...
      if (!label_map.empty()) {
        ...
        if (iter == std::end(label_map)) {
          std::cout << absl::Substitute("Missing labels for patient id ? $0\n",
                                        patient_id);
          abort();
        }
      ...
      uint32_t length_delta = (patient_lengths.at(start_index).second -
      ...
      std::shuffle(std::begin(batch.drop_flags), std::end(batch.drop_flags),
                   batch_rng);
      ...
      found = batch.iter->process_patient(
          patient_id, [&](absl::CivilDay birth_day, uint32_t age,
            ...
            for (auto code_with_value : observations_with_values) {
              ...
              if (lab_data != nullptr) {
                ...
                if (code_with_value.is_text) {
                  value_token = lab_data->get_index(code_with_value.text_value);
                } else {
                  value_token =
                      lab_data->get_index(code_with_value.numeric_value);
                }
            ...
            auto last = std::unique(std::begin(batch.positive_codes_per_day),
                                    std::end(batch.positive_codes_per_day));
            ...
            std::sort(std::begin(batch.lab_codes), std::end(batch.lab_codes));
            ...
            if (day_index != length && !is_drop) {
              ...
              for (int pos_i = 0; pos_i < 50; pos_i++) {
                ...
                position[pos_i * 4 + 2] =
                    std::sin(((float)age / 365) * parent.rates[pos_i]);
                position[pos_i * 4 + 3] =
                    std::cos(((float)age / 365) * parent.rates[pos_i]);
              }
              ...
              for (uint32_t code : batch.positive_codes_per_day) {
                if (auto valid_code = parent.get_valid_code(code)) {
                  if (*valid_code >= parent.threshold) {
                    batch.codes1.push_back(*valid_code - parent.threshold);
                    added_one1 = true;
                  } else {
              ...
              if (age != 0) {
                uint64_t patient_index = i * max_length + current_offset;
                special_targets(batch, patient_id, patient_index,
                                parent.icd_root, age, age, censor_age);

                // for (uint32_t target = 0; target < parent.num_valid_targets;
                // target++) {
                ...
                //             censor_mask = true;
                //             event_mask = false;
                //         } else if (batch.target_ages[target] <= age) {
                //             // Already happened
                //             time = 1;
                //             censor_mask = false;
                //             event_mask = false;
                ...
                //             time = batch.target_ages[target] - age;
                //             censor_mask = false;
                //             event_mask = true;
                //         }
                //     }

                //     size_t index = max_length * parent.num_valid_targets * i
                //     + parent.num_valid_targets * current_offset + target;
      ...
      if (day_index != length + 1) {
        std::cout << "Day index should count up to length + 1? " << day_index
                  << " " << length << " " << patient_id << " "
                  << patient_lengths.at(start_index + i).second << std::endl;
        abort();
      }
  ...
  py::dict next() {
    ...
    py::handle patient_id_numpy = PyArray_SimpleNewFromData(
        1, patient_id_numpy_dims, NPY_INT64, current_batch->patient_ids.data());
    ...
    py::handle day_index_numpy = PyArray_SimpleNewFromData(
        2, day_index_numpy_dims, NPY_INT64, current_batch->day_index.data());
    ...
    npy_intp day_info_numpy_dims[] = {(npy_intp)current_batch->day_info.size(),
                                      5};
    py::handle day_info_numpy = PyArray_SimpleNewFromData(
        2, day_info_numpy_dims, NPY_FLOAT32, current_batch->day_info.data());

    result["rnn"] =
        py::make_tuple(codes_numpy, offsets_numpy, codes1_numpy, offsets1_numpy,
                       day_info_numpy, pos_encoding_numpy, lengths_numpy);
    ...
    if (label_map.empty()) {
      ...
      // target_dims, NPY_BOOL, current_batch->censor_masks.data()); py::handle
      // target_event_numpy = PyArray_SimpleNewFromData(3, target_dims,
      // NPY_BOOL, current_batch->event_masks.data());
      ...
      result["task"] = py::make_tuple(target_indices_numpy, target_labels_numpy,
                                      target_factors_numpy);

    } else {
      // npy_intp label_indices_numpy_dims[] = {(npy_intp)
      // current_batch->label_indices.size()}; py::handle label_indices_numpy =
      ...
      // result["labeler"] = py::make_tuple(label_indices_numpy, labels_numpy);
    }
