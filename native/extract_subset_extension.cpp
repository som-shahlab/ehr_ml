...
std::vector<uint32_t> generate_random_indices(size_t upper_bound, size_t target) {
...
class Reader2Writer {
  public:
    Reader2Writer(const char * src_path,
      ...
      std::vector<uint32_t> target_indices = generate_random_indices(total_patients, target_num_patients);
