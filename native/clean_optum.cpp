 ...
 public:
  Cleaner(const char* path) : reader(path, false), iterator(reader.iter()) {
...

    {
      ...
      for (const auto& entry : items) {
        ...
        for (const auto& term : terms) {
          result.push_back(temp_dictionary.map_or_add(term, entry.second));
        }
      ...
      for (const auto& entry : lost_counts) {
        lost_entries.push_back(std::make_pair(-entry.second, entry.first));
    ...
    std::cout << "Final Dictionary size " << final_dictionary.size()
              << std::endl;
    std::cout << "Final Value dictionary size " << final_value_dictionary.size()
              << std::endl;
          ...
          patient_id, [&](absl::CivilDay birth_date, uint32_t age,
            ...
            for (const auto& obs : observations) {
              for (const auto& remapped : remap_dict[obs]) {
                record.observations.push_back(std::make_pair(age, remapped));
              }
            }
            ...
            for (const auto& obs_with_value : observations_with_values) {
              for (const auto& remapped_code :
                ...
                if (obs_with_value.is_text) {
                  ...
                  new_obs.numeric_value = obs_with_value.numeric_value;
                }
