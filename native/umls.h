...
class UMLS {
 ...
 private:
  ...
  absl::flat_hash_map<std::string, std::pair<std::string, std::string>>
      aui_to_code_map;
  absl::flat_hash_map<std::string, std::vector<std::string>> aui_to_parents_map;
  ...
  load_code_to_aui_map(std::string umls_path) {
    ...
    std::set<std::string> valid_sabs = {"HCPCS", "CPT", "SRC",   "ICD10CM",
                                        "ATC",   "LNC", "MTHHH", "ICD10PCS"};
    ...
    while (std::getline(infile, line)) {
      ...
      if (aui == "A1415709" ||
        ...
        auto [iter, added] =
            result.insert(std::make_pair(std::make_pair(sab, code), aui));
        if (!added) {
          std::cout << "Got duplicate aui " << code << " " << sab << " " << aui
  ...
  load_aui_to_parents_map(std::string umls_path,
      const absl::flat_hash_map<
          std::string, std::pair<std::string, std::string>>& aui_to_code_map) {
