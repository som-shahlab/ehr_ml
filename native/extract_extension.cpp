...
std::optional<std::string> try_to_recover(
    const UMLS& umls,
  ...
  if (new_id_candidates.size() == 0) {
    ...
    std::cout << "Odd " << info.vocabulary_id << " " << info.concept_code << " "
              << concept_id << " " << new_id_candidates.size() << std::endl;
    return std::nullopt;
  } else {
    const auto& info = *table.get_info(new_id_candidates[0]);

    for (std::string terminology : map_terminology_type(info.vocabulary_id)) {
...
std::vector<uint32_t> compute_subwords(
  ...
  if (aui_to_subwords_map.find(aui) == std::end(aui_to_subwords_map)) {
    ...
    if (std::find(std::begin(results), std::end(results),
                  dictionary.map_or_add("SRC/V-SRC")) == std::end(results)) {
      std::cout << "AUI " << aui << " has no root " << std::endl;

      for (const auto& item : results) {
        std::cout << "Got " << dictionary.get_word(item).value() << std::endl;
      }
...
void create_ontology(std::string_view root_path, std::string umls_path, std::string cdm_location) {
  ...
  std::set<std::string> recorded_date_code_types = {
      "ATC", "CPT4", "DRG", "Gender", "HCPCS", "ICD10CM", "ICD10PCS", "LOINC"};
  ...
  for (uint32_t i = 0; i < entries.size(); i++) {
    ...
    if (result == std::nullopt && (parts[0] == "CPT4" || parts[0] == "ICD10CM")) {
      // Manually try to recover by using the OMOP hierarchy to map to something
      // useful.
      ...
      if (!opt_concept_id) {
        std::cout << "Could not get inverse concept id " << word << std::endl;
        abort();
      }
    ...
    if (result == std::nullopt) {
      subwords = {
          ontology_dictionary.map_or_add(absl::Substitute("NO_MAP/$0", word))};
    } else {
      subwords = compute_subwords(*result, umls,
                                  aui_to_subwords_map, code_to_parents_map,
                                  ontology_dictionary);
    }
...
void create_index(std::string_view root_path) {
  ...
  for (uint32_t patient_id : extract.get_patient_ids()) {
    ...
    bool found = iterator.process_patient(
        ...
        [&codes](
            ...
            const std::vector<ObservationWithValue>& observations_with_values) {
    ...
    if (!found) {
      std::cout << absl::Substitute("Could not find patient id $0", patient_id)
                << std::endl;
      abort();
    }
  ...
  for (auto& item : patients_per_code) {
    ...
    size_t max_needed_size =
        streamvbyte_max_compressedbytes(patient_ids.size()) + sizeof(uint32_t);
...
absl::flat_hash_map<uint64_t, PatientInfo> get_patient_data(std::string location) {
  ...
  csv_iterator(
      person_file.c_str(), columns, '\t', {}, true, [&i, &result](const auto& row) {
        int64_t person_id;
        int year_of_birth;
    ...
    if (info.vocabulary_id == "NDC") {
      ...
      for (const auto& relationship : table.get_relationships(concept_id)) {
        if (relationship.relationship_id == "Maps to") {
          std::optional<ConceptInfo> other_info = table.get_info(relationship.other_concept);
          if (!other_info) {
              continue;
          }
      ...
      if (iter == std::end(rxnorm_to_atc)) {
        ...
        for (const auto& ancestor : table.get_ancestors(rxnorm_code)) {
          ...
          if (anc_info && (anc_info->vocabulary_id == "RxNorm" ||
...
void merge_intermediates(
  ...
  for (size_t i = 0; i < files.size(); i++) {
    ...
    std::cout << absl::Substitute("$0 normal terms and $1 value terms for $2\n",
                                  dict_entries.size(), value_entries.size(),
                                  files[i]);
...
void perform_omop_extraction(std::string omop_source_dir, std::string umls_dir, std::string gem_dir, std::string target_directory) {


  int error = mkdir(target_directory.c_str(), 0700);

  if (error == -1) {
      std::cout << absl::Substitute("Could not make result directory $0, got error $1\n", target_directory, std::strerror(errno)); exit(-1);
  }
  ...
  if (error == -1) {
      std::cout << absl::Substitute("Could not make result directory $0, got error $1\n", root_path, std::strerror(errno)); exit(-1);
  }
  ...
  converter_threads.push_back(generate_converter_thread(omop_source_dir, root_path, "demo",
  DemographicsConverter(), patient_data));
  converter_threads.push_back(generate_converter_thread(omop_source_dir, root_path, "visit",
  VisitConverter(), patient_data));
  converter_threads.push_back(generate_converter_thread(omop_source_dir, root_path, "measure",
  MeasurementConverter(), patient_data));
  converter_threads.push_back(generate_converter_thread(omop_source_dir, root_path, "drug",
  StandardConceptTableConverter("drug_exposure.csv.gz",
  "drug_exposure_start_date", "drug_source_concept_id"), patient_data));
  converter_threads.push_back(generate_converter_thread(omop_source_dir, root_path, "death",
  StandardConceptTableConverter("death.csv.gz", "death_date",
  "death_type_concept_id"), patient_data));
  converter_threads.push_back(generate_converter_thread(omop_source_dir, root_path, "cond",
  StandardConceptTableConverter("condition_occurrence.csv.gz",
  "condition_start_date", "condition_source_concept_id"), patient_data));
  converter_threads.push_back(generate_converter_thread(omop_source_dir, root_path, "proc",
  StandardConceptTableConverter("procedure_occurrence.csv.gz",
  "procedure_date", "procedure_source_concept_id"), patient_data));
  converter_threads.push_back(generate_converter_thread(omop_source_dir, root_path, "device",
  StandardConceptTableConverter("device_exposure.csv.gz",
  "device_exposure_start_date", "device_source_concept_id"), patient_data));
  converter_threads.push_back(generate_converter_thread(omop_source_dir, root_path, "obs",
