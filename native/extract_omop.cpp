...
const char* location = "/scratch/labs/shahlab/OPTUM_CDMv5/Optum_CDM";
...
absl::flat_hash_map<uint64_t, PatientInfo> get_patient_data() {
  ...
  csv_iterator(
      person_file.c_str(), columns, ',', {}, [&i, &result](const auto& row) {
        int64_t person_id;
        int year_of_birth;
        ...
        result.insert(
            std::make_pair(person_id, PatientInfo(person_id, i++, birth_date)));
      });
...
class Converter {
 public:
  ...
  void augment_day(TermDictionary& dictionary, TermDictionary& value_dictionary,
...
class MeasurementConverter : public Converter {
 public:
  ...
  std::vector<std::string_view> get_columns() const {
    return {"person_id", "measurement_date", "measurement_source_concept_id",
...
void write_timeline(const char* filename, const TermDictionary& dictionary,
  ...
  while (true) {
    ...
    std::sort(std::begin(record.observations), std::end(record.observations));
    ...
    ages.erase(std::unique(std::begin(ages), std::end(ages)), std::end(ages));
    ...
    for (uint32_t age : ages) {
      ...
      while (current_observation_index < record.observations.size() &&
             record.observations[current_observation_index].first == age) {
      ...
      while (
          ...
          record.observationsWithValues[current_observation_with_values_index]
                  .first == age) {
        auto [code, value] =
            record.observationsWithValues[current_observation_with_values_index]
                .second;
      ...
      buffer[num_obs_with_value_index] = current_observation_with_values_index -
  ...
  writer.add_str("num_patients", (const char*)&num_patients, sizeof(uint32_t));
...
std::thread generate_converter_thread(
    std::string_view root_file, std::string_view name, C convert,
    const absl::flat_hash_map<uint64_t, PatientInfo>& patient_data) {
  return std::thread([convert, root_file, name, &patient_data]() {
    std::string file_name = absl::Substitute("$0_$1", root_file, name);
    run_converter(file_name, convert, patient_data);
  });
}

template <typename C>
void run_converter(
    std::string_view filename, const C& convert,
    const absl::flat_hash_map<uint64_t, PatientInfo>& patient_data) {
  ...
  csv_iterator(
      person_file.c_str(), convert.get_columns(), ',', {},
      [&](const auto& row) {
        num_rows++;
...
class GEMMapper {
 public:
  GEMMapper() {
    helper("2018_I9gem.txt", diag_convert_map);
    helper("gem_i9pcs.txt", proc_convert_map);
  }

  void helper(
      std::string_view path,
      absl::flat_hash_map<std::string, std::vector<std::string>>& target) {
    std::string full_filename = absl::Substitute("$0/$1", gem_location, path);
...
convert_to_concept_strings(const TermDictionary& terms,
  ...
  auto [mapper, counts] = terms.decompose();
  std::vector<std::vector<uint32_t>> converter(counts.size());
  ...
  for (uint32_t code = 0; code < counts.size(); code++) {
    const std::string& term = mapper[code];

    uint32_t concept_id;
    if (!absl::SimpleAtoi(term, &concept_id)) {
      std::cout << absl::Substitute("Could not parse supposed concept_id $0\n",
                                    term);
    }
    ...
    if (info.vocabulary_id == "NDC") {
      ...
      for (const auto& relationship : table.get_relationships(concept_id)) {
        if (relationship.relationship_id == "Maps to") {
          ConceptInfo other_info = table.get_info(relationship.other_concept);
      ...
      if (rxnorm_codes.size() == 0) {
        // std::cout << absl::Substitute("Could not find any rxnorm code for
        // $0\n", concept_id);
        continue;
      }
      ...
      if (iter == std::end(rxnorm_to_atc)) {
        ...
        for (const auto& ancestor : table.get_ancestors(rxnorm_code)) {
          ...
          if (anc_info.vocabulary_id == "RxNorm" ||
            ...
            for (const auto& relationship : table.get_relationships(ancestor)) {
        ...
        atc_codes.erase(std::unique(std::begin(atc_codes), std::end(atc_codes)),
                        std::end(atc_codes));

        // if (atc_codes.size() == 0) {
        //     std::cout << absl::Substitute("Could not find any atc codes for
        //     $0\n", rxnorm_code);
        // }
    ...
    for (const auto result : results) {
      ...
      converter[code].push_back(
          new_term_dictionary.map_or_add(final, counts[term]));
    }
  }
...
void merge_intermediates(const std::vector<std::string>& files,
                         const std::string& target) {
  GEMMapper gem;

  std::cout << absl::Substitute("Processing the concept table\n");

  ConceptTable concepts = construct_concept_table(location);
  ...
  for (size_t i = 0; i < files.size(); i++) {
    ...
    auto [dict_mapper, dict_count] = new_dict.decompose();
    auto [val_dict_mapper, val_dict_count] =
        reader.get_value_dictionary().decompose();

    std::cout << absl::Substitute("$0 normal terms and $1 value terms for $2\n",
                                  dict_count.size(), val_dict_count.size(),
                                  files[i]);
    ...
    for (const auto& entry : dict_count) {
      dict_counts[entry.first] += entry.second;
    }

    for (const auto& entry : val_dict_count) {
  ...
  auto iter = [&]() {
    ...
    uint64_t next_patient_original_id = std::numeric_limits<uint64_t>::max();
    ...
    for (size_t i = 0; i < patient_record_indices.size(); i++) {
      ...
      if (index < readers[i].get_patient_ids().size() &&
        ...
        next_patient_original_id = readers[i].get_original_patient_ids()[index];
      }
    }
    ...
    for (size_t i = 0; i < patient_record_indices.size(); i++) {
      ...
      if (index < readers[i].get_patient_ids().size() &&
        ...
        bool found = iterators[i].process_patient(
            next_patient_id, [&](absl::CivilDay birth_date, uint32_t age,
              ...
              for (auto obs_with_value : observations_with_values) {
                for (uint32_t converted : converter[obs_with_value.code]) {
                  ...
                  record.observationsWithValues.push_back(
                      std::make_pair(age, obs_with_value.encode()));
                }
              }
            });

        if (!found) {
          std::cout << absl::Substitute("Could not find patient_id $0\n",
                                        next_patient_id);
          exit(-1);
        }
      }
...
int main() {
  std::string root_directory =
      "/labs/shahlab/projects/ethanid/ehr_ml/native/result7";

  std::string root_path = absl::Substitute("$0/temp", root_directory);

  // int error = mkdir(root_directory.c_str(), 0700);

  // if (error == -1) {
  //     std::cout << absl::Substitute("Could not make result directory $0, got
  //     error $1\n", root_directory, std::strerror(errno)); exit(-1);
  // }

  // std::cout << absl::Substitute("Starting to download patient data\n");
  // auto patient_data = get_patient_data();
  // std::cout << absl::Substitute("Found $0 patients\n", patient_data.size());

  // std::vector<std::thread> converter_threads;
  // converter_threads.push_back(generate_converter_thread(root_path, "demo",
  // DemographicsConverter(), patient_data));
  // converter_threads.push_back(generate_converter_thread(root_path, "visit",
  // VisitConverter(), patient_data));
  // converter_threads.push_back(generate_converter_thread(root_path, "measure",
  // MeasurementConverter(), patient_data));
  // converter_threads.push_back(generate_converter_thread(root_path, "drug",
  // StandardConceptTableConverter("drug_exposure.csv.gz",
  // "drug_exposure_start_date", "drug_source_concept_id"), patient_data));
  // converter_threads.push_back(generate_converter_thread(root_path, "death",
  // StandardConceptTableConverter("death.csv.gz", "death_date",
  // "death_type_concept_id"), patient_data));
  // converter_threads.push_back(generate_converter_thread(root_path, "cond",
  // StandardConceptTableConverter("condition_occurrence.csv.gz",
  // "condition_start_date", "condition_source_concept_id"), patient_data));
  // converter_threads.push_back(generate_converter_thread(root_path, "proc",
  // StandardConceptTableConverter("procedure_occurrence.csv.gz",
  // "procedure_date", "procedure_source_concept_id"), patient_data));
  // converter_threads.push_back(generate_converter_thread(root_path, "device",
  // StandardConceptTableConverter("device_exposure.csv.gz",
  // "device_exposure_start_date", "device_source_concept_id"), patient_data));
  // converter_threads.push_back(generate_converter_thread(root_path, "obs",
  // StandardConceptTableConverter("observation.csv.gz", "observation_date",
  // "observation_source_concept_id"), patient_data));
  ...
  DIR* d = opendir(root_directory.c_str());
  if (d) {
    ...
    while (dir != nullptr) {
      ...
      files.push_back(absl::Substitute("$0/$1", root_directory, filename));
    }
    closedir(d);
  }

  std::string target = absl::Substitute("$0_final", root_directory);

  merge_intermediates(files, target);
}
