...
absl::flat_hash_map<uint64_t, PatientInfo> get_patient_data() {
    ...
    csv_iterator(person_file.c_str(), columns, ',', {},
...
int main() {
    ...
    // if (error == -1) {
    //     std::cout << absl::Substitute("Could not make result directory $0,
    ...
    // converter_threads.push_back(generate_converter_thread(root_path,
    // "measure", MeasurementConverter(), patient_data));
    ...
    // converter_threads.push_back(generate_converter_thread(root_path,
    // "device", StandardConceptTableConverter("device_exposure.csv.gz",
    // "device_exposure_start_date", "device_source_concept_id"),
    // patient_data));
    ...
    if (d) {
        ...
        while (dir != nullptr) {
            ...
            files.push_back(
                absl::Substitute("$0/$1", root_directory, filename));
        }
        closedir(d);
    }
