...
struct PatientDay {
  ...
  absl::Span<const ObservationWithValue> get_observations_with_values() const {
...
class TimelineReader {
 public:
   ...
   public:
    ...
    Patient next() {
      if (index == patient_ids.size()) {
        ...
        return reader->get_patient(patient_ids[index++], start_date, end_date);
      }
    }
  ...
  TimelineReaderIterator get_patients(
      ...
      std::optional<absl::CivilDay> end_date) {
    if (!patient_ids) {
      patient_ids = std::vector<uint32_t>(std::begin(reader.get_patient_ids()),
