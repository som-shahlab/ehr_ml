 ...
 public:
  RxNorm() {
    ...
    rxnorm_util("RXNSAT.RRF", [&](const auto& columns) {
      ...
      auto key = std::make_pair(std::string(actual_sab), std::string(atv));
    ...
    rxnorm_util("RXNREL.RRF", [&](const auto& columns) {
      ...
      if (valid_relationships.find(rela) == std::end(valid_relationships)) {
        return;
      }

      auto [iter, added] = children_map.insert(
          std::make_pair(std::string(parent_cui), std::vector<std::string>()));
      (void)added;

      iter->second.push_back(std::string(child_cui));
    });

    absl::flat_hash_map<std::string, std::vector<std::string>> all_children_map;
    auto get_all_children = [&](const std::string& rxcui) {
      return get_all_children_helper(rxcui, children_map, all_children_map);
    };
      ...
      if (child_iter != std::end(children_map)) {
        for (const std::string& child : child_iter->second) {
          for (const std::string& subchildren :
               get_all_children_helper(child, children_map, all_children_map)) {
