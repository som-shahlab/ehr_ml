    ...
    if (strm.avail_out == 0 || ret == LZMA_STREAM_END) {
      ...
      for (size_t i = 0; i < end_index; i++) {
        ...
        if (current_char == '\n') {
          // Need to purge
          if (last_newline == 0) {
            f(absl::string_view(last_line.data(), last_line.size()));
            last_line.clear();
          } else {
      ...
      if (last_newline != 0) {
        last_line.insert(std::end(last_line), outbuf.data() + last_newline,
...
void split_into_fields(std::vector<absl::string_view>& fields,
  ...
  for (size_t i = 0; i < text.size(); i++) {
    ...
    if (in_quotes) {
      ...
      if (c == ',') {
        fields.push_back(text.substr(last_comma + 1, (i - last_comma) - 2));
        last_comma = i + 1;
      } else if (c == '"') {
        in_quotes = true;
      }
  ...
  stream_zx(filename, [&](absl::string_view text) {
    ...
    if (!parsed_header) {
      for (size_t j = 0; j < field_names.size(); j++) {
        ...
        if (!found) {
          std::cout << "Could not find field name " << field_names[j] << " for "
                    << filename << std::endl;
